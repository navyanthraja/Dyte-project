						...
						<h4><b class="text-info">Add Mark</b></h4>
							...
							if(isset($_POST['submit'])){
								...
								if($result){
									...
									$qry="SELECT MAIL,NAME FROM STUDENT WHERE RNO='$regno'";
									$equery=$db->query($qry);
									if(!empty($equery) && $equery->num_rows > 0){
									
										while($emname=$equery->fetch_assoc()){
											$to_email=$emname["MAIL"];
											$to_name=$emname["NAME"];
									}
								}
								
									
									$subject = "$etype Marks uploaded for $sub";
									$body = "
									Hello,$to_name
									Your marks for $sub are $mark
									";
									$headers = "From: sender email";

									/*if (mail($to_email, $subject, $body, $headers)) {
										echo "<h4 class='m-2 p-2 text-light bg-success'>Email sent Successully</h4>";
									} 
									else {
										echo "<h4 class='m-2 p-2 text-light bg-danger'>Email sending failed</h4>";
									}*/
